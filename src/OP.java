...
class OP {
	...
	if (index >= 0) {
	    ...
	    // recurse
	    /*
	     * May 2018 from modified SYmbolic persues
	     */
	    else
	    {
	    	/*int x = config[1][index]-1; 
	    	DD d1 = DD.zero;
	    	//if(x<dd.getChildren().length)
	    	{
	    		 d1 =  dd.getChildren()[x];//<--- this is the problem that we don't have a third node 
	    	}
...
//-------------------reorder for costobj--------
    ...
    public static DD reorderCost(DD dd) {
    	...
    	int[] varSet = dd.getVarSet();
    	int highestVar = varSet[varSet.length-1];
    	int[][] config = new int[2][1];// why it is 2,1 dimensions? 
    	config[0][0] = highestVar;
    	
    	DD[] children = new DD[Global.varDomSize[highestVar-1]];// since the varDomainSize is [3,2,,3,2], so this size will be 3 now 
    	//DD[] children0 = new DD[2];
    	//for(int x=0; x<2; x++)// x is number of objectives in this case it is 2 TODO: we need to make it generalized 
    	{
    		...
    		for (int i=0; i<Global.varDomSize[highestVar-1]; i++) 
    		{
    			...
    			DD restDd = OP.restrict(dd,config);
    			children[i] = OP.reorder(restDd);
    			//children[i] = OP.reorderCost(restDd);
    		}
    		
    		//children0[x]=DDnode.myNew(highestVar,children);
    	}
    	//return DDnode.myNew(highestVar,children0);
    	return DDnode.myNew(highestVar,children);
        }
