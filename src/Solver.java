...
class Solver implements Serializable {
	...
	if (generate) {
	    ...
	    POMDP pomdp = new POMDP(spuddfile,debug,w);
	    ...
	    if (generate) {
		pomdp.solve(nRounds, numBelStates, maxBelStates, episodeLength, threshold, explorProb, nIterations, maxAlphaSetSize, basename, multinits,w);
	    }
		...
		if (obj instanceof POMDP) {
		    ...
		    pomdp.readFromFile(spuddfile,debug,w);
		    /*
